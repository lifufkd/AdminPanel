...
class ChangeProfile(UserControl):
    ...
    def build(self):
        #ЗНАЧЕНИЯ#
        user_name = TextField(label='Имя', value='First Name') #имя пользователя уже подставленное (подставить значение value='text')
        user_pass = TextField(label='Пароль', password=True)
        user_pass_confirm = TextField(label='Подтвердите пароль', password=True)
        user_login = TextField(label='Логин', value='Логин') #логин уже подставленный
