...
class LoadData:
    ...
    def application(self):
        ...
        raw_data = self.__db.get_data(
            f'SELECT number, application_type, payment_type, application_status, hospitalized, status, date_create FROM application ORDER BY date_create DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
            ...
            for row in range(len(rows)):
                if row in pointer.keys():
                    if row == 6:
                        ...
                        l1.append(word_wrap(
                            self.__db.get_data(f'SELECT title FROM {pointer[row]} WHERE id = {rows[row]}', ())[0][0],
                            max_len))
                else:
    ...
    def area(self):
        ...
        data = list()
        raw_data = self.__db.get_data(
            f'SELECT area, region FROM area ORDER BY area DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows)):
    ...
    def clinics(self):
        ...
        max_len = 30  # перенос слов по 15 символов
        raw_data = self.__db.get_data(
            f'SELECT name, med_profiles, site, phone_number, email FROM hospital ORDER BY name DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows)):
    ...
    def ksg(self):
        ...
        raw_data = self.__db.get_data(
            f'SELECT code, title, price, ratio_switch, id FROM ksg ORDER BY code DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
    ...
    def med_profile(self):
        ...
        raw_data = self.__db.get_data(
            f'SELECT med_profile, id FROM med_profile ORDER BY med_profile DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows)):
    ...
    def mkb(self):
        ...
        raw_data = self.__db.get_data(
            f'SELECT code, title, id FROM mkb ORDER BY code DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
    ...
    def region(self):
        ...
        max_len = 400  # перенос слов по 15 символов
        raw_data = self.__db.get_data(
            f'SELECT title, id FROM region ORDER BY title DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows)):
    ...
    def users(self):
        ...
        c_page = 1  # выбор страницы, в поле ввода по умолчанию поставить .value = 1
        raw_data = self.__db.get_data(
            f'SELECT id, role, full_name, photo, date_create, email, phone_number, agent, blocked FROM users ORDER BY id DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows)):
    ...
    def service(self):
        ...
        raw_data = self.__db.get_data(
            f'SELECT code, title, id FROM service ORDER BY code DESC LIMIT 15 OFFSET {(c_page - 1) * 15}',
            ())
        for rows in raw_data:
            l1 = []
            for row in range(len(rows) + 1):
